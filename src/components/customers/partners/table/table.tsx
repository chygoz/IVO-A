...
import { Partner } from "@/actions/business";
...
function PartnerTable({ partners }: PartnerTableProps) {
  const [dateRange, setDateRange] = React.useState("last90");
  return (
    <Card className="p-5">
      <div className="flex items-center mb-6">
        <h3 className="font-bold text-text text-[28px] ">All Partners</h3>
                  ...
                  <TableCell className="">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="text-[#898F9B]">{partner?.description}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="">
                    <div className="flex items-center gap-2">
                      <Avatar>
                        ...
                        <AvatarFallback>
                          {getInitials(
                            `${partner.owner.firstName} ${partner.owner.lastName}`
                          )}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="text-[#898F9B]">{partner.owner.email}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableHead>
                    <Status
                      variant={partner.verified ? "success" : "destructive"}
                      text={partner.verified ? "verified" : "unverified"}
                    />
                  </TableHead>
                </TableRow>
              ))}
