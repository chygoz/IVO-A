...
function NewWithdrawalForm({ banks }: NewWalletProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess(data) {
      router.replace(
        `/dashboard/accounts/withdraw-accounts/verify-otp?requestId=${data.requestId}`
      );
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {!shouldFetchName({
            ...
            <div className="rounded-xl text-primary capitalize p-2 mt-4 flex items-center gap-2">
              ...
              >
                <rect width="16" height="16" rx="8" fill="#20483F" />
                <path
          ...
          {bankNameData?.data.account_name ? (
            <Button disabled={mutation.isPending} className="w-full mt-8">
