...
import { deleteBlank } from "@/actions/blanks/delete.blank";
import { formatCurrency } from "@/lib/utils";
import { IBlank, IVariant, IGallery } from "@/actions/blanks/types";
          ...
          {images.map((image, index) => (
            ...
            >
              <div
                ...
                className={`relative h-20 w-20 cursor-pointer overflow-hidden rounded-md border-2 transition hover:opacity-90 ${selectedImage?.url === image.url
                  ? "border-primary"
                  : "border-transparent"
                  }`}
              >
...
const BlankDetailComponent = () => {
  const params = useParams();
  const router = useRouter();
  const [product, setProduct] = useState<IBlank | null>(null);
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      setLoading(true);
      const response = await getBlank(params.slug as string);
      if (!response?.data) {
        setLoading(false)
        return
      }
      console.log(response)
      setProduct(response.data as IBlank);
      setLoading(false)
    };
  ...
  const mutation = useMutation({
    mutationFn: (id: string) => deleteBlank(id),
    onSuccess: () => {
      toast.success("Blank deleted successfully");
      router.push("/dashboard/resellers/blanks");
      setIsDeleteModalOpen(false);
    },
    onError: (error) => {
      toast.error("Failed to delete blank", {
  ...
  if (loading) {
    return (
      <div className="container mx-auto py-8" data-testid="loading-skeleton">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <Skeleton className="aspect-square w-full rounded-lg lg:col-span-2" />
          <div className="space-y-4">
  ...
  return (
    <>
      <ConfirmationModal
        ...
        title="Are you sure you want to delete this blank?"
        description="This action cannot be undone. This will permanently delete the blank and all its data."
        confirmText="Delete"
          ...
          <div className="flex items-center gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" asChild>
                    <Link
                      href={`/dashboard/resellers/blanks/${product._id}/preview`}
                      target="_blank"
                    >
                ...
                <TooltipContent>Preview blank</TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" asChild>
                    <Link
                      href={`/dashboard/resellers/blanks/${product._id}/edit`}
                    >
                      <Edit className="h-4 w-4" />
                    </Link>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>Edit blank</TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>Delete blank</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
        </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  ...
                                  <div className="font-medium">
                                    {product.shipping?.dimension
                                      ? `${product.shipping.dimension.length} x ${product.shipping.dimension.width} × ${product.shipping.dimension.height}`
                                      : "—"}
                                  </div>
                                </div>
                              </div>
                ...
                <Card>
                  ...
                  <CardFooter className="flex justify-between gap-2">
                    ...
                    <Button className="w-full" asChild>
                      <Link href={`/dashboard/resellers/blanks/${product._id}/edit`}>
              ...
              >
                <Card>
                  ...
                  <CardFooter>
                    ...
                    >
                      <Link
                        href={`/dashboard/resellers/blanks/${product._id}/edit?tab=seo`}
                      >
                        Edit SEO
                      </Link>
                    </Button>
...
export default BlankDetailComponent;
